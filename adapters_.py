...
Original file is located at
    https://colab.research.google.com/drive/1kXGWDdggDNqpjiNZSM8Il72QPGho_PDK

# **Training of AST and ConvNext-tiny with the ASVSpoof19 LA dataset applying an Adapter to the last 2 layers of AST and to every block in the last stage of ConvNext-tiny**
"""
...
# ============================
# Mount & Extract .flac Files
# ============================
drive.mount('/content/drive')

os.makedirs("/content/flac", exist_ok=True)

# Use -j to flatten file structure
!unzip -o -j "/content/drive/MyDrive/LA/train/flac.zip" -d "/content/flac"

# Verify extraction
flacs = [f for f in os.listdir("/content/flac") if f.endswith(".flac")]
print(f"Found {len(flacs)} .flac files in /content/flac")
...
class ASVspoofDataset(Dataset):
    def __init__(self, protocol_file, audio_dir, model_type='ast',
        ...
        print(f"✅ Loaded {len(self.df)} samples from protocol file: {protocol_file}")
    ...
    def __getitem__(self, idx):
        ...
        file_stem = row['file']
        if not file_stem.endswith('.flac'):
            file_stem += '.flac'

        wav_path = os.path.join(self.audio_dir, file_stem)
...
def build_convnext_with_adapters():
    ...
    blocks = last_stage.blocks if hasattr(last_stage, "blocks") else []
    ...
    for block in blocks:
        if hasattr(block, 'dwconv'):
            ...
            raise AttributeError("No depthwise conv layer found.")
...
# ============================
# Training Entry Point with Saving
# ============================
def run_training(model_type='ast'):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    protocol_path = "/content/sample_data/train/ASVspoof2019.LA.cm.train.trn.txt"
    audio_path = "/content/flac"

    dataset = ASVspoofDataset(protocol_path, audio_path, model_type=model_type)
    ...
    # Save trained model with adapters
    save_dir = "/content/drive/MyDrive/LA/models"
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f"{model_type}_adapter_only.pth")
...
# ====================================
# 2. Imports (match your training code)
# ====================================
import torch
import pandas as pd
import numpy as np
...
def run_inference(model, protocol_file, audio_dir, model_type, device):
    model.eval()
    dataset = ASVspoofDataset(protocol_file, audio_dir, model_type=model_type)
...
# 8. Run Evaluation
# ====================================
evaluate_model("ast", "/content/drive/MyDrive/LA/models/ast_adapter_only.pth")
evaluate_model("convnext", "/content/drive/MyDrive/LA/models/convnext_adapter_only.pth")
...
# Set CM & ASV score file paths (DEV)
# =============================
cm_score_file = '/content/cm_scores_dev_convnext.txt'      # CM scores: 3-column <utt> <label> <score>
asv_score_file = '/content/asv_dev.txt'               # ASV scores: 3-column <utt> <label> <score>
